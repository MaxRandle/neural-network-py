...
class NN:
    ...
    def backPropogate(self, y, a):
        ...
        """
        #output layer is handled differently to hidden layers
        error = y - a[-1]
        gradient = np.multiply(error, self.dg(a[-1]))
        delta = a[-2].T * gradient
        self.weights[-1] += delta * self.alpha
        self.biases[-1] += gradient * self.alpha

        ls = len(self.weights)
        for n in range(1, ls-1)[::-1]:
            error = gradient * self.weights[n+1].T
            gradient = np.multiply(error, self.dg(a[n]))
            delta = a[n-1].T * gradient
            self.weights[n] += delta * self.alpha
            self.biases[n] += gradient * self.alpha
    ...
    def train(self, train_x, train_y, epochs):
        ...
        for n in range(epochs):
            ...
            for i in range(len(train_x)):
                ...
                y_error = train_y[i] - activations[-1]
                self.backPropogate(train_y[i], activations)
                

            
            #if (n == 1) or (n == epochs//4) or (n == 2*epochs//4) or (n == 3*epochs//4) or (n == epochs-1):
            #if (n % epochs//4) == 0:
            if (n % 250 == 0):
                print("Training loss: " + str(np.mean(np.square(y_error))))
